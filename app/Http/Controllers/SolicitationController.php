...
class SolicitationController extends Controller
{
    ...
    public function index()
    {
        ...
        $solicitations = SolicitationModel::with(['colaborator', 'timeTracking']);

        // Aplicar filtros de busca
        if (request('search')) {
            $search = request('search');
            $solicitations = $solicitations->where(function ($query) use ($search) {
                $query->whereHas('colaborator', function ($subQuery) use ($search) {
        ...
        switch ($sortBy) {
            case 'colaborator_name':
                $solicitations = $solicitations->join('collaborators', 'solicitations.colaborator_id', '=', 'collaborators.id')
        ...
        if ($solicitations->currentPage() > $solicitations->lastPage() && $solicitations->lastPage() > 0) {
            ...
            if (!request()->ajax()) {
                return redirect()->route('solicitations.index', array_merge(request()->query(), ['page' => $solicitations->lastPage()]));
            }
    ...
    public function approve(Request $request, $id): RedirectResponse
    {
        ...
        try {
            ...
            $timeTracking->update([
                'time_start' => $solicitation->new_time_start ?? $timeTracking->time_start,
                'time_finish' => $solicitation->new_time_finish ?? $timeTracking->time_finish,
            ]);
            ...
            return redirect()->back()->with('error', 'Erro ao aprovar solicitação.');
        }
    }
